...
class TestLinkShortenerUnit:
    ...
    def test_shorten_link_valid_url(
        self, client: TestClient, sample_url: str, cleanup_redis: pytest.Fixture
    ) -> None:
    ...
    def test_shorten_link_with_custom_alias(
        self, client: TestClient, sample_url: str, cleanup_redis: pytest.Fixture
    ) -> None:
        """Тест сокращения URL с кастомным алиасом"""
        custom_alias = "my-custom-link"
        response = client.post(
            "/shorten", json={"original_url": sample_url, "custom_alias": custom_alias}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    def test_shorten_link_duplicate_custom_alias(
        self, client: TestClient, sample_url: str, cleanup_redis: pytest.Fixture
    ) -> None:
    ...
    def test_redirect_nonexistent_link(
        self, client: TestClient, cleanup_redis: pytest.Fixture
    ) -> None:
        """Тест редиректа несуществующего короткого URL"""
        response = client.get("/nonexistent123")
        assert response.status_code == 404

    def test_get_stats_nonexistent_link(
        self, client: TestClient, cleanup_redis: pytest.Fixture
    ) -> None:
    ...
    def test_shorten_link_invalid_url(
        self, client: TestClient, cleanup_redis: pytest.Fixture
    ) -> None:
...
class TestLinkShortenerModels:
    ...
    def test_link_create_model_valid(self) -> None:
    ...
    def test_link_create_model_with_alias(self) -> None:
    ...
    def test_link_response_model(self) -> None:
